...
Every task creates a fresh `.runs/<run_id>/` directory copied from the selected project. A Run contains:
...
`run.json` is atomically updated through a temporary file and `os.replace`. `evidence.ndjson` stores the complete event records in append order.

### 5. Verification Layer

The current contract contains four equal-weight gates:

| Gate | Required observation |
| --- | --- |
| Baseline failure | a pre-patch test command fails and is recorded as baseline evidence |
...
Only `complete_run()` can emit `Task completed`. If a model returns `finish` early, the system emits `Completion blocked`, reports missing evidence, and keeps the Live loop inside the bounded control budget.
